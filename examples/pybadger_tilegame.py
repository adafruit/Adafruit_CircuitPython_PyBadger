"""
This example shows a basic tilemap game. It builds on the Multiple Tile Grid example in the
displayio learn guide:
...
You must copy tilegame_assets/ directory to your CIRCUITPY device to use this example.
"""
import time
import random
import board
...
sprite_sheet, palette = adafruit_imageload.load("tilegame_assets/sprite_sheet.bmp",
                    ...
                    if 'entity' in TILES[tile_name].keys() and TILES[tile_name]['entity']:
                        ...
                        if tile_name == "player":
                            ...
                            if (x, y) not in GAME_STATE['ENTITY_SPRITES_DICT']:
                                ...
                                GAME_STATE['ENTITY_SPRITES_DICT'][x, y] = [new_entity_obj]
                            else:
                                # append the entity to the existing list in the dictionary
                                GAME_STATE['ENTITY_SPRITES_DICT'][x, y].append(new_entity_obj)
...
def can_player_move(direction):
    if direction == UP:
        _tile = (GAME_STATE['PLAYER_LOC'][0], GAME_STATE['PLAYER_LOC'][1] - 1)
        return TILES[GAME_STATE['CURRENT_MAP'][_tile[0], _tile[1]]]['can_walk']
    ...
    if direction == LEFT:
        _tile = (GAME_STATE['PLAYER_LOC'][0] - 1, GAME_STATE['PLAYER_LOC'][1])
        return TILES[GAME_STATE['CURRENT_MAP'][_tile[0], _tile[1]]]['can_walk']
            ...
            if tile_name in TILES.keys():
                ...
                if (x + CAMERA_OFFSET_X, y + CAMERA_OFFSET_Y) in GAME_STATE['ENTITY_SPRITES_DICT']:
                    ...
                    if tile_name != "player":
                        ...
                        for entity_obj_at_tile in \
                                GAME_STATE['ENTITY_SPRITES_DICT'][x + CAMERA_OFFSET_X, y + CAMERA_OFFSET_Y]:
                            ...
                            # add the index of the entity sprite to the drew_entities list so we know not to hide it.
...
with open("tilegame_assets/game_message_background.bmp", "rb") as game_message_background:
    ...
    while True:
        ...
        if GAME_STATE["STATE"] == STATE_PLAYING:
            ...
            if x_offset != 0 or y_offset != 0:
                ...
                moving_to_coords = (GAME_STATE['PLAYER_LOC'][0] + x_offset, GAME_STATE['PLAYER_LOC'][1] + y_offset)

                # tile name of the spot player is moving to
                moving_to_tile_name = GAME_STATE['CURRENT_MAP'][moving_to_coords[0], moving_to_coords[1]]
                ...
                if moving_to_coords in GAME_STATE['ENTITY_SPRITES_DICT']:
                    ...
                    for entity_obj in GAME_STATE['ENTITY_SPRITES_DICT'][moving_to_coords]:
                        ...
                        if "before_move" in TILES[entity_obj["map_tile_name"]].keys():
                            print("calling before_move %s, %s, %s" % (
                                moving_to_coords, GAME_STATE['PLAYER_LOC'], entity_obj))
                            # call the before_move behavior function act upon it's result
                            if TILES[entity_obj["map_tile_name"]]['before_move'](moving_to_coords,
                                                                                 GAME_STATE['PLAYER_LOC'], entity_obj,
                                                                                 GAME_STATE):
            ...
            if GAME_STATE["STATE"] == STATE_MAPWIN:
                ...
                if GAME_STATE["MAP_INDEX"] >= len(MAPS):
                    ...
                    GAME_STATE['STATE'] = STATE_WAITING
                    show_splash("You beat this level\n =D \nCongratulations. \nStart Next?", 0x29c1cf)
                    load_map(MAPS[GAME_STATE["MAP_INDEX"]])
